...
let alti1 thm prop =
 ...
 let impi th left_jgmt =
  ...
  match jgmt with
  | J (x, p) ->
      if List.exists (function v -> v = jgmt) ass && x = y then
        ImpI
          ( th,
            ( 
...
let diae thm1 thm2 y =
  ...
  | J(x, Dia(a)), J(z, b) ->
    ...
    if List.length matching_assumptions = 2 &&
      List.exists (function v -> v = R(x,y) ) matching_assumptions &&
      List.exists (function v -> v = J(y, a)) matching_assumptions then
        let ass2 = List.filter (function v -> v <> R(x,y) && v <> J(y, a)) ass2 in
